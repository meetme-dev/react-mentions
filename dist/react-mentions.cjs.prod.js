...
function ch2pattern(ch) {
  if (/[가-힣]/.test(ch)) {
    var chCode = ch.charCodeAt(0) - 44032;
    if (chCode % 28 > 0) return ch;
    var begin = 28 * Math.floor(chCode / 28) + 44032, end = begin + 27;
  ...
  if (/[ㄱ-ㅎ]/.test(ch)) {
    var con2syl = {
      ...
      "ㅃ": "빠".charCodeAt(0),
      "ㅅ": "사".charCodeAt(0)
    }, _begin = con2syl[ch] || 588 * (ch.charCodeAt(0) - 12613) + con2syl["ㅅ"], _end = _begin + 587;
...
var lettersDiacritics = [ {
  ...
  var display = ignoreAccents ? normalizeString(str) : str, regex = createMatcher(ignoreAccents ? normalizeString(substr) : substr), match = display.match(regex);
